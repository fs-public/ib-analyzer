...
export const loadAndValidateConfig = () => {
  ...
  assert(validate(personalData), `Personal config not conforming to AJV schema - ${JSON.stringify(validate.errors)}`, true)
  ...
  for (const source of sources) {
    if (!source.schema || !source.transformation) {
      ...
      assert(source.schema.length === source.transformation.length, `Source mismatch between schema and transformation lengths for ${source.filename}.`, true)
