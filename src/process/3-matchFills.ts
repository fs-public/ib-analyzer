...
const matchFillsPerCloseOrder = (matchableOpens: Order[], currentClose: Order): Fill[] => {
  ...
  assert(unfilled === 0, "Unimplemented situation: filled some, cannot match fully. Possibly a missing or mismatched trade.")
  ...
  if (fills.filter((f) => f.timetestApplied).length > 0) currentClose.code = codeAddFlag(currentClose.code, CODES.CUSTOM.TIMETEST_PASSED)
