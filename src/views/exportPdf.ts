...
const getOneTable = (view: ViewDefinition): TemplateTable => {
  ...
  return {
    title: view.name,
    columns: Object.keys(results[0].table[0] || results[1].table[0] || results[2].table[0] || {}).map((key) => _.startCase(key)),
    rows: titledResults,
  }
}
...
const exportPdf = async () => {
  const data = {
    ...
    statistics: (["categories", "currencies", "symbols", "activeSymbols", "years"] as (keyof typeof env.data.sets)[])
      .map((key) => `${_.startCase(key)} (<b>${env.data.sets[key].size}</b>): ${Array.from(env.data.sets[key] as Set<string>).join(", ")}`)
